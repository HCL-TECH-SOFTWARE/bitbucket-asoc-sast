...
"""
Windows entry point for the AppScan on Cloud SAST Bitbucket Pipe.
...
class AppScanOnCloudSAST(AppScanOnCloudSASTBase):
    """Windows-specific SAST pipe."""

    def _get_reports_dir(self):
        """On Windows the reports directory is relative to the clone-dir parent."""
        cloneParent = os.path.dirname(os.path.abspath(self.cloneDir))
        return self._safe_path_join(cloneParent, REPORTS_DIR)

    def _resolve_appscan_path(self, saclientPath, dirEntry):
        """On Windows the appscan binary path is prefixed by cwd."""
        return self._safe_path_join(self.cwd, saclientPath, dirEntry, "bin", APPSCAN_BIN_NAME)
